...
FRONTEND_DIST_TGZ="${FRONTEND_DIST_TGZ:-/tmp/frontend-dist.tgz}"
...
# Frontend is built on GitHub Actions (server has ~400MB RAM — too small for Vite).
if [ ! -f "${FRONTEND_DIST_TGZ}" ]; then
  fail "Prebuilt frontend archive not found: ${FRONTEND_DIST_TGZ}"
fi

log "Publishing prebuilt frontend from ${FRONTEND_DIST_TGZ}..."
sudo rm -rf "${WEB_STAGING}"
sudo mkdir -p "${WEB_STAGING}"
sudo tar -xzf "${FRONTEND_DIST_TGZ}" -C "${WEB_STAGING}"
